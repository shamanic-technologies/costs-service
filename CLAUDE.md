...
```
`--ours` is safe here only because `main` carries nothing but promote-squashes of commits `staging` already has — confirm with `git log origin/staging..origin/main --oneline` first, and after the push verify `git diff origin/main --stat` shows ONLY your intended files.

⚠️ **Resolve EVERY conflicted path, not the one git names last — `git add -A` happily commits conflict markers.** The merge prints one `CONFLICT` line per path but only the last is on screen when you reach for the fix, so `git checkout --ours <that one file> && git add -A && git commit` stages `<<<<<<<`/`=======`/`>>>>>>>` into the others. Nothing catches it: the commit succeeds, `git status` is clean, and `README.md`/`CLAUDE.md` are not compiled or tested, so `npm test` and `npm run build` stay green on a tree carrying markers in prose. Take the list from `git status --short` (`UU`/`AA`/`AU`/`UA`) rather than from the merge output, and gate the commit on `grep -rn '^<<<<<<<\|^>>>>>>>' CLAUDE.md README.md src/ tests/` returning nothing. Observed 2026-08-30 (v0.51.0 promote): three conflicts, one resolved, markers committed into both docs and caught only by the grep. Recovery is `git checkout origin/staging -- <files>` + a second commit — the tree is what matters, so verify `git diff origin/staging --stat` is EMPTY before pushing the fast-forward. The open promote PR then flips to `MERGEABLE` on its own.
...
**A NEW MODEL launched already regime-priced carries ONE version, dated at the vendor's stated instant — do NOT fabricate a `2025-01-01` uniform version for it.** `deepSeekModelCosts` takes `current` as OPTIONAL and a `regimeFrom` override for exactly this: V4 Flash and V4 Pro each carry a pre-schedule uniform version because DeepSeek really did charge one rate at every hour before 2026-08-16T16:00Z, but a model released afterwards never had a regime-free rate, so inventing one would back-date a price to before the model existed. DeepSeek V4.1 Flash (released 2026-09-10, priced from `DEEPSEEK_V4_1_FLASH_PRICING_FROM` = 2026-09-10T04:00Z, API model id `deepseek-flash`) is the precedent: six names, one version each, reusing the unchanged `DEEPSEEK_{PEAK,OFF_PEAK}_HOURS_UTC` windows. It is a NEW model rather than a re-price — its peak cache-miss input is $0.3/1M against V4 Flash's $0.44/1M — so every `deepseek-v4-flash-*` row was left untouched. Same call for a vendor DISCONTINUING a model: V4 Pro is routed to V4.1 Flash and billed at the Flash price from 2026-09-14T04:00Z, and its rows stay frozen exactly as written (correct until then, and spend already declared must keep resolving) — consumers move to the new names, the catalog appends nothing. Regression: `tests/unit/deepseek-v4.1-flash-costs.test.ts`.
...
**A `unit: "1M tokens"` row stores VENDOR CENTS PER TOKEN — `$/MTok / 10,000` before the markup — and the check that catches a mis-scaled one is a SIBLING MODEL, never the row itself.** The `1M tokens` label is cosmetic (consumers report a raw token count), so the literal is `($/MTok x 100 cents) / 1,000,000 tokens`: $1/MTok is `0.0001000000` raw / `0.0005000000` stored at 5x, $10/MTok is `0.0010000000` / `0.0050000000`. An exponent slip writes a plausible-looking decimal that no test, no `check:readme` and no read-path assertion can flag, because every one of them only compares the row to itself — the row is internally consistent and the catalog is silently 10x off for that model. What DOES catch it: the same vendor dollar figure must map to the same stored value on every model, so a new row's value is verified against an EXISTING row at a known vendor price (Haiku 4.5 $1, Sonnet 4.6 $3, Opus 4.6 $5) and the ratio must match the vendors' ratio. A cached-input row is not an independent check — it is usually derived from its own base row, so both slip together. Regression: `tests/unit/token-price-scale.test.ts` asserts stored-vs-vendor-dollars for every token row it lists, including anchors nobody is editing; add each new model's three rows to that table in the same PR. Cost 2026-09-09 (v0.55.0 -> v0.56.0): Claude Fable 5.1 and OpenAI GPT-6 Astra shipped all six rows at `$/MTok / 100,000`, so Fable 5.1 — which lists at TEN times Haiku 4.5's input price — was stored at Haiku's exact number and both models under-billed 10x until the correction.

**Store markup = `COST_RISK_MULTIPLIER` × `COST_PROFIT_MULTIPLIER` (two factors, default 2 × 2.5 = 5×).** Every seed value is `raw × COST_DEFAULT_MULTIPLIER` where `COST_DEFAULT_MULTIPLIER = COST_RISK_MULTIPLIER * COST_PROFIT_MULTIPLIER` (`src/db/seed.ts`). Risk covers cost under-estimation; profit is the store margin. `applyCostRiskMultiplier(raw)` applies the product; an explicit 2nd-arg override REPLACES the whole markup (profit does NOT stack on an override — the override path is test-only today). Changing either factor reprices EVERY cost at once via the append-only path above (new `now()`-dated rows on deploy) — and only `marked-up` lines: `pass-through` never carries a markup, so a factor change must leave every routed line at exactly the vendor rate. Profit went 2 → 2.5 (4× → 5×) in 2026-08 when the cold-email infrastructure spend moved onto our own fixed costs: the markup on what we still rebill carries the unit economics the retired lines used to. It went 2.5 → 3 (5× → 6×) right after, as a pure store-margin raise, then back 3 → 2.5 (6× → 5×) as a margin cut — same append-only mechanics every time, nothing changed about risk. The lever moves in both directions and the procedure is identical. When you touch a factor you must also: regenerate the README catalog values, the seed-derived test literals (`tests/unit/providers-costs.test.ts` + per-provider `*-costs.test.ts` + `tests/integration/{stripe-fees,seed-append-history}.test.ts`), leaving fixture-inserted integration literals (`insertTestProviderCost`) untouched. The README column is not hand-edited: `check:readme` compares a SORTED multiset of `(name, value)`, so regenerate it by NAME, not by INDEX — build a per-name FIFO of the seed's values in declaration order and pop one for each table row as you walk it. The table's rows are NOT index-aligned with `SEED_PROVIDERS_COSTS` (they diverge around row 42 today, and a name with several versions has one row per version), so a positional zip asserts its way straight into a mismatch. Test literals are cheapest to fix from the suite's own output (each failure prints the correct value), but patch only where the old literal is UNIQUE on its line — a value repeated across two entries of the same map, or a `toBe(5)` whose digit also appears in the comment, needs the anchor widened by hand.

**Rescale a test literal ONLY where the suite reported it — never by sweeping a value map across the tests, because a RAW VENDOR CELL and a MARKED-UP EXPECTED VALUE are the same shape and the uniqueness rule above cannot tell them apart.** The old→new map you build from the markup change is a map over PRICES, and several per-provider tests hold the vendor's own cell in an identically-shaped table (`const VENDOR = { "<name>": "0.0000600000" }`, the `unVatted` map in `deepseek-china-vat.test.ts`) and compute the expected value through `applyCostRiskMultiplier` at assert time. Those tests PASS both before and after the factor change — which is exactly why a sweep reaches them: they are not in the failure list, so nothing tells you the line was edited, and the literal was unique on its line so the uniqueness guard stayed green. The result is a test that now asserts against a vendor price the vendor never charged, and it goes red on the NEXT model addition rather than on this PR. The safe procedure is line-scoped: parse `expected '<actual>' to be '<stale>'` plus the `❯ <file>:<line>` beneath it out of the run output, and replace only on that line. If you do sweep, re-run the suite afterwards and treat any file that was edited but was NOT failing as a corruption to revert. Cost 2026-09-15 (v0.58.0, 6× → 5×): a map-shaped sweep rewrote three raw vendor cells across `deepseek-v4.1-flash-costs.test.ts` and `deepseek-china-vat.test.ts`; the suite caught them only because the rescaled raw fed a marked-up assertion in the same file.

**Verifying the reprice in production: a price value is NEVER evidence on its own, because the table is append-only and every markup era is still in it.** Polling for `cost_per_unit_in_usd_cents = '5.0000000000'` on a repriced name matches the row the 5× era wrote weeks ago, so the poll reports the deploy as landed on tick 1 against a container that has not been rebuilt. Bound the predicate to the NEWEST row AND to a fresh `effective_from` (`ORDER BY effective_from DESC LIMIT 1` + `effective_from > now() - interval '2 hours'`), and emit your own verdict token rather than matching on the value. The whole-catalog check is cheaper and unambiguous anyway: `SELECT count(*) FROM providers_costs WHERE effective_from::date = current_date` must equal the number of marked-up in-force keys (106 at v0.58.0), and the same query filtered to `pricing_basis='pass-through' AND cost_per_unit_in_usd_cents <> '1.0000000000'` must be 0. Then exercise `GET /v1/platform-prices/:name` inside the container for one line of each basis — a row in the table is state, the served price is behaviour.
