...
describe("Google Gemini 3.7 Flash unit costs (flash-pro alias upgrade)", () => {
  it("registers google-flash-3.7-tokens-input at $1.50/1M input tokens (standard tier)", () => {
    const row = SEED_PROVIDERS_COSTS.find((c) => c.name === "google-flash-3.7-tokens-input");
    expect(row).toBeDefined();
    expect(row!.provider).toBe("google");
    expect(row!.providerDomain).toBe("google.com");
    expect(row!.type).toBe("Input tokens (Gemini 3.7 Flash)");
  ...
  it("registers google-flash-3.7-tokens-output at $7.50/1M output tokens (standard tier)", () => {
    const row = SEED_PROVIDERS_COSTS.find((c) => c.name === "google-flash-3.7-tokens-output");
    expect(row).toBeDefined();
    expect(row!.provider).toBe("google");
    expect(row!.providerDomain).toBe("google.com");
    expect(row!.type).toBe("Output tokens (Gemini 3.7 Flash)");
    expect(row!.unit).toBe("1M tokens");
    expect(row!.planTier).toBe("pay-as-you-go");
    expect(row!.billingCycle).toBe("monthly");
  ...
  it("resolves both rows against the active google platform cost (plan_tier + billing_cycle match)", () => {
    ...
    // A mismatch here would make GET /v1/platform-prices/google-flash-3.7-tokens-* 404.
    for (const name of ["google-flash-3.7-tokens-input", "google-flash-3.7-tokens-output"]) {
  ...
  it("leaves the Gemini 3.6 Flash rows untouched (additive change)", () => {
    const input = SEED_PROVIDERS_COSTS.find((c) => c.name === "google-flash-3.6-tokens-input");
    const output = SEED_PROVIDERS_COSTS.find((c) => c.name === "google-flash-3.6-tokens-output");
