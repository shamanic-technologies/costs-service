...
// Quantity = fee in cents; unit price = 1 cent base × COST_DEFAULT_MULTIPLIER (risk 2 × profit 2 = 4)
// = "4.0000000000". Org is charged 4× the actual Stripe fee, matching the
...
describe("Stripe fee cost names", { timeout: 30_000 }, () => {
  ...
  for (const { name, type } of STRIPE_COST_NAMES) {
    it(`GET /v1/platform-prices/${name} resolves at 4¢/USD-cent`, async () => {
